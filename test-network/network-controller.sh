...
function networkUp() {
  ...
  # Change the name of the Admin cert to a generic one so Hyperledger Explorer will work
  WORKING_DIRECTORY=${PWD}
  cd ./organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp/keystore/
  mv *_sk admin_sk
...
function cleanUp() {
    ...
    rm -rf system-genesis-block/*.block organizations/peerOrganizations organizations/ordererOrganizations ../*-cc/chaincode-java/build ../*-cc/chaincode-java/.gradle
...
if [ "$MODE" == "up" ]; then
  ...
  infoln "Cleaning Up Generated CA Files"
else
  printHelp
  exit 1
fi
